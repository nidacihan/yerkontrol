...
class Ui_MainWindow(object):
    ...
    def uzaklik_hesapla(self, lat1, lon1, lat2, lon2):
        R = 6371
    ...
    def update_labels(self):
        baslangic_lat = self.iha.location.global_frame.lat
        baslangic_lon = self.iha.location.global_frame.lon
        mevcut_lat = self.iha.location.global_frame.lat
        mevcut_lon = self.iha.location.global_frame.lon

        simdi = datetime.now()
        tarih = simdi.strftime("%Y-%m-%d")
        saat = simdi.strftime("%H:%M:%S")

        yukseklik_iha = self.iha.location.global_relative_frame.alt
        hava_hizi_iha = self.iha.airspeed
        gps_hizi_iha = self.iha.groundspeed
        batarya_iha = self.iha.battery.level
        mod_durumu_iha = self.iha.mode.name
        gps_sayisi_iha = self.iha.gps_0.satellites_visible
        pitch_iha = self.iha.attitude.pitch
        yaw_iha = self.iha.attitude.yaw
        roll_iha = self.iha.attitude.roll
        arm_iha = "Aktif" if self.iha.armed else "Pasif"
        telemetri_iha = "Telemetri bağlantısı aktif" if self.iha.last_heartbeat else "Telemetri bağlantısı pasif"
        uzaklik_iha = self.uzaklik_hesapla(baslangic_lat, baslangic_lon, mevcut_lat, mevcut_lon)
        baud_rate = 57600

        hostname = socket.gethostname()
        ip_adresi = socket.gethostbyname(hostname)
        self.ip.setText(f"IP Adresi: {ip_adresi}")
        self.baud.setText(f"Baud: {baud_rate}")
        self.start_time = time.time()
        ucus_suresi = time.time() - self.start_time
        self.ucus_suresi.setText((f"Uçuş Süresi: {ucus_suresi:.2f}saniye"))
        self.yukseklik.setText(f"Yükseklik: {yukseklik_iha:.2f} m")
        self.hava_hizi.setText(f"Hava Hızı: {hava_hizi_iha:.2f} m/s")
        self.gps_hizi.setText(f"GPS Hızı: {gps_hizi_iha:.2f} m/s")
        self.batarya.setText(f"Batarya: %{batarya_iha}")
        self.mod_durumu.setText(f"Uçuş modu: {mod_durumu_iha}")
        self.gps_sayisi.setText(f"Gps Uydu Sayısı: {gps_sayisi_iha}")
        self.pitch.setText(f"Pitch: {pitch_iha:.2f}Rad")
        self.yaw.setText(f"Row: {yaw_iha:.2f}Rad")
        self.roll.setText(f"Roll: {roll_iha:.2f}Rad")
        self.arm.setText(f" Arm: {arm_iha:}")
        self.telemetri.setText(f"telemetri: {telemetri_iha:}")
        self.uzaklik.setText(f"Uzaklık: {uzaklik_iha:.2f} metre")
        self.saat.setText(f"Saat: {saat}")
        self.tarih.setText(f"Tarih: {tarih}")
...
"color: rgb(0, 0, 0);")
    ...
    def takeoff(self):
        self.iha.simple_takeoff(10)
        self.update_output("takeoff başlatıldı")
        while True:
            mevcut_irtifa = self.iha.location.global_relative_frame.alt
    ...
    def land(self):
        self.iha.mode = VehicleMode("LAND")

        if self.iha.location.global_relative_frame.alt == 0:
           self.update_output("iniş gerçekleşti")

    def stabilize(self):
        self.iha.mode = VehicleMode("STABILIZE")
        self.update_output("stabilize moda geçiş yapıldı")

    def loiter(self):
        self.iha.mode = VehicleMode("LOITER")
        self.update_output("loiter moda geçiş yapıldı")

    def rtl(self):
        self.iha.mode = VehicleMode("RTL")
        self.update_output("RTL moda geçiş yapıldı")

    def disarm_ol(self):
        if self.iha.location.global_relative_frame.alt >= 3:
            self.update_output("irtifa üç metrenin üstüne çıkarsa disarm edilemez")
            return

        self.iha.armed = False
